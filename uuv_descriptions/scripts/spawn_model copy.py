...
class SpawnModel:
    ...
    def parseUserInputs(self):
        ...
        for i in range(0, len(sys.argv)):
            ...
            if sys.argv[i] == "-urdf":
                if self.sdf_format:
                    print(
                        "Error: you cannot specify both urdf and sdf format xml, must pick one"
                    )
                    sys.exit(0)
                else:
            ...
            if sys.argv[i] == "-param":
                if len(sys.argv) > i + 1:
                    if self.file_name != "" or self.database_name != "":
                        print(
                            "Error: you cannot specify file name if parameter or database name is given, must pick one source of model xml"
                        )
                        sys.exit(0)
                    else:
            ...
            if sys.argv[i] == "-file":
                if len(sys.argv) > i + 1:
                    if self.param_name != "" or self.database_name != "":
                        print(
                            "Error: you cannot specify parameter if file or database name is given, must pick one source of model xml"
                        )
                        sys.exit(0)
                    else:
            ...
            if sys.argv[i] == "-database":
                if len(sys.argv) > i + 1:
                    if self.param_name != "" or self.file_name != "":
                        print(
                            "Error: you cannot specify parameter if file or parameter name is given, must pick one source of model xml"
                        )
                        sys.exit(0)
                    else:
    ...
    def callSpawnService(self):
        ...
        if not self.wait_for_model == "":
            rospy.Subscriber(
                "%s/model_states" % self.gazebo_namespace,
                ModelStates,
                self.checkForModel,
            )
            r = rospy.Rate(10)
        ...
        if len(self.file_name) > 0:
            ...
            if os.path.exists(self.file_name):
                if os.path.isdir(self.file_name):
                    rospy.logerr(
                        "Error: file name is an invalid path, %s", self.file_name
                    )
                    sys.exit(0)
        ...
        if self.package_to_model:
            model_xml = re.sub(
                "<\s*mesh\s+filename\s*=\s*([\"|'])package://",
                "<mesh filename=\g<1>model://",
                model_xml,
            )
        ...
        if None not in self.initial_geo and None not in self.ref_geo and use_pymap:
            ...
            rospy.loginfo(
                "Geodetic coordinates: (%.7f, %.7f, %.2f)",
                self.initial_geo[0],
                self.initial_geo[1],
                self.initial_geo[2],
            )
            rospy.loginfo(
                "Geodetic reference: (%.7f, %.7f, %.2f)",
                self.ref_geo[0],
                self.ref_geo[1],
                self.ref_geo[2],
            )
        ...
        rospy.loginfo(
            "Initial position wrt the world frame:"
            " (%.2f, %.2f, %.2f)"
            % (
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        )  # make this a print incase roscore has not been started
        sm = SpawnModel()
        sm.parseUserInputs()
        sm.callSpawnService()
